...
Datx.prototype.lookup = function(ip) {
  ...
  var floor = this.buffer.readInt32LE(partition + 4) * 9 + 262144;
  var ceil = this.len - 262144 - 4;

  // binary search
  var low = 0;
  var high = Math.floor((ceil - floor) / 9);
  var mid;

  while (low <= high) {
    mid = (low + high) >>> 1;
    var offset = floor + mid * 9 + 4;
    var a = this.buffer.readInt32BE(offset) >>> 0;
    var b = this.buffer.readInt32BE(offset + 9) >>> 0;
    ...
    if (b < ip) {
      ...
      var recordOffset = (index & 0x00FFFFFF) + this.len - 262144;
